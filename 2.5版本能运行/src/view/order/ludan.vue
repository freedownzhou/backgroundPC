...
<template>
  <div>
  <div class="mix-page list-page box-shadow" v-if="btn_jump">
    ...
    >
      ...
      <div class="modal-content">
        <Checkbox-group v-model="customColumnModal.checkeds">
          <Checkbox v-for="(item,index) in tableFormatArray" v-bind:key="index" :label="item.key" >{{item.title}}</Checkbox>
        </Checkbox-group>
          ...
          <div class="mt10">
            <RadioGroup v-model="customColumnModal.radio" style="padding-left:20px">
              ...
              <div class="clearfix mt10">
                ...
                <span class="pull-left fs12 txt-color-666 ml22">注：选择此项，则选择一次有效并清除系统储存，下次将使用系统默认显示列。</span>
              </div>
            </RadioGroup>
          </div>
        </div>
      ...
      <div class="modal-content">
        <div :model="dataExportModel" class="clearfix">
          <div class="pull-left" style="width:50%">
              ...
              <RadioGroup v-model="dataExportModel.export" @on-change="checkExportGroupChange" :style="{paddingLeft:'20px'}">
          ...
          <div class="pull-right" style="width:50%" >
              <div>
<!--                // :indeterminate为ture就是默认全选-->
                <Checkbox
                    :indeterminate="dataExportModel.indeterminate"
                    :value="dataExportModel.checkAll"
                    @click.prevent.native="handleCheckAll">自定义导出列：</Checkbox>
              </div>
              <Checkbox-group v-model="dataExportModel.checkeds" @on-change="checkAllGroupChange" class="pl30 custom-export-column">
                <Checkbox v-for="(item,index) in tableFormat" v-show="item.check" :label="item.key" v-bind:key="index" style="display:block" class="mt10">{{item.title}}</Checkbox>
              </Checkbox-group>
          </div>
        </div>
      </div>
...
// 剛打開页面时columnData.length == 0且cookieValue为false，所以会进入foreach循环，这时会吧原column数据转换为可识别的数据
...
// 当选择了定制列，重新进入这个函数。columnData.length已经是有数值了的，所以进入customTableFormat.length == 0 && columnData.length > 0判断条件。
//      columnData是你选择的定制列内容，2个循环将原tableFormat（tableFormat是所有表头数据，在created里面赋值的）数据中包含有选择的columnData定制列内容全部push局部变量customTableFormat中
      if (customTableFormat.length == 0 && columnData.length > 0) {
        // alert(3)
        console.log(customTableFormat)
        tableFormat.forEach(element => {
...
// 将整理好的新的customTableFormat（也就是表头数据）赋值给tableFormat
      that.tableFormat = customTableFormat;
      if (that.customColumnModal.radio === '1') {       // cookie存储防止自定义表头后想增减一些必须要全部重新设置。可以从已经定义好的数组中增减。
    ...
    customColumnConfirm(){
      ...
      var s = that.initTabelFormat(that.customColumnModal.checkeds)
      console.log(s)
  ...
  created() {
    ...
    if(this.$route.path !== "/ludan"){this.btn_jump=false;}
  },
  mounted() {
  }
};
