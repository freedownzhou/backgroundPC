    ...
    <div v-if=alltwo_show>
      <h4 class="title-bar clearfix">
        ...
        <div class="extra pull-right">
          <a class="clearfix pl5 pr5 theme-btn-color pull-left lh18"  @click="ModelType('一级子菜单')">
            <i class="pull-left iconfont icon-addto fs14"></i>
            <p class="pull-left ml5 fs12">创建一级子菜单</p>
          </a>
  ...
  export default {
    ...
    methods: {
      ...
      MenuSon(msg){
        this.alltwo_show = false;
        console.log(msg)
        let date2={parentOneId:msg.id,parentId:msg.parent_id,name:msg.menu_name}
        this.$router.push({name: "二级子菜单",params: date2});
      },
      // 创建按钮函数
      ModelType(value){
        let data = {
    ...
    created() {
      ...
      if(this.$route.name == '一级子菜单'){this.alltwo_show = true}
      this.parentId =this.$route.params.parentId,
      this.Mone_name =this.$route.params.name,
      this.getTableData();
    },
    mounted() {
    }
