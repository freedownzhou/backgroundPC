        ...
        <div v-if="ModelType == '权限'" class="minH">
          <Form :rules="rulePermissions" :model="permission" class="pl30 pr30" :label-width="80" ref="permission">
            ...
            <FormItem label="权限描述" prop="describe">
              <Input v-model="permission.describe"></Input><p class="WenZi">长度2-125个字符</p>
            </FormItem>
            <FormItem label="权限类型">
            ...
            <FormItem label="平台编码" prop="platform_code">
              <Input v-model="permission.platform_code" :placeholder='permission.platform_code' disabled></Input>
            </FormItem>
            <FormItem label="权限编码" prop="power_code">
              <Input v-model="permission.power_code" :placeholder='permission.power_code'></Input>
            </FormItem>
            <FormItem label="状态">
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            if (this.FromName === 'userFrom') {
              ...
              Api.updatePower(that.permission).then(res=>{
                if (res && res.code == 0) {
      ...
      this.$root.Bus.$on("Modify-click", messages=>{
        ...
        for(var p in messages[0]){
          ...
          if(that.userFrom.hasOwnProperty(p)){
            ...
            that.permission.describe = messages[0].power_describe;
            that.permission.powerId = messages[0].id;
