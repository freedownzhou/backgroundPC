        ...
        <div id="root-app" class="root-app in-homepage">
            <div class="sidebar-app app">
                ...
                <div>
                    ...
                    <div class="side-nav-box" v-for="(item,index) in authMenu" :key="item.index" v-model="authMenu" v-if="item.isShow == 1">
                        <div class="side-nav-item pb15">
                            <div class="side-nav-item-title">{{item.menuName}}</div>
                            <router-link :to="_item.menuPath" tag="li" class="menu-item" active-class="current"
                                v-for="(_item, twoIndex) in item.children" :key="twoIndex" v-if="_item.isShow == 1" v-show="twoIndex < item.showCount">
                                <span>{{_item.menuName}}</span>
                            </router-link>
                            <div v-if="item.children.length > item.minCount" class="fold-btn-bar">
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            let authMenu = JSON.parse(this.Util.getMenu());
            authMenu.forEach((item, i) => {
                let ch = item.children.length;
                if (ch > 0) {
                    authMenu[i].show = false;
                    authMenu[i].count = ch;
                    authMenu[i].minCount = ch > 3 ? 3 : 2;
                    authMenu[i].showCount = ch > 3 ? 3 : 2;
                }
            });
